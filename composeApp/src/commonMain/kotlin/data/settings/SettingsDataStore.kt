...
class SettingsDataStore(private val dataStore: DataStore<Preferences>) {
    ...
    val isDarkTheme: Flow<Boolean> = dataStore.data.map { it[PreferencesKeys.IS_DARK_THEME] ?: false }
    suspend fun updateDarkTheme(isDark: Boolean) {
        dataStore.edit { it[PreferencesKeys.IS_DARK_THEME] = isDark }
    }

    val savePath: Flow<String> = dataStore.data.map { it[PreferencesKeys.SAVE_PATH] ?: "" }
    suspend fun updateSavePath(path: String) {
        dataStore.edit { it[PreferencesKeys.SAVE_PATH] = path }
    }
    ...
    // Signer settings
    val signerJarPath: Flow<String> = dataStore.data.map { it[PreferencesKeys.SIGNER_JAR_PATH] ?: "" }
    val lastApkPath: Flow<String> = dataStore.data.map { it[PreferencesKeys.LAST_APK_PATH] ?: "" }

    suspend fun updateSignerSettings(java: String, jar: String, apk: String) {
        dataStore.edit {
            it[PreferencesKeys.JAVA_PATH] = java
            it[PreferencesKeys.SIGNER_JAR_PATH] = jar
            it[PreferencesKeys.LAST_APK_PATH] = apk
        }
    }

    // Global Variables
    val globalVariables: Flow<Map<String, String>> = dataStore.data.map { preferences ->
        val jsonStr = preferences[PreferencesKeys.GLOBAL_VARS] ?: "{}"
        try {
    ...
    suspend fun updateGlobalVariables(vars: Map<String, String>) {
        dataStore.edit {
            it[PreferencesKeys.GLOBAL_VARS] = Json.encodeToString(vars)
        }
    }
    ...
    private object PreferencesKeys {
        ...
        val SIGNER_JAR_PATH = stringPreferencesKey("signer_jar_path")
        val LAST_APK_PATH = stringPreferencesKey("last_apk_path")
        val GLOBAL_VARS = stringPreferencesKey("global_vars")
    }
}
